...
def obtener_info_linux(ip):
    try:
        ...
        # Obtener el navegador predeterminado (en sistemas Linux)
        navegador = subprocess.check_output(["xdg-settings", "get", "default-web-browser"]).strip().decode()
        ...
        # Obtener aplicaciones instaladas (en sistemas Linux usando dpkg)
        salida = subprocess.check_output(["dpkg", "--get-selections"]).decode()
        print("Aplicaciones Instaladas:")
        print(salida)

        # Obtener usuarios del sistema (en sistemas Linux)
        salida = subprocess.check_output(["cut", "-d", ":", "-f", "1", "/etc/passwd"]).decode().splitlines()
        ...
        print(f"Error al obtener la información en Linux: {str(e)}")

# Ejemplo de uso para Linux:
ip = input("Introduce una dirección IP de Linux: ")
obtener_info_linux(ip)
